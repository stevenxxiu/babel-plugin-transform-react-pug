...
function getTypeFromValidator(validator) {
  if (validator.type) {
    ...
    if (
      ...
      return "$ReadOnlyArray<" + getTypeFromValidator(validator.chainOf[1].each) + ">";
    }
    if (
      validator.chainOf.length === 2 &&
...
output.push(`type Location = {start: {line: number, column: number}, end: {line: number, column: number}};`);
output.push(``);
...
output.push(`type JSXValue = JSXText | JSXExpressionContainer | JSXSpreadChild | JSXElement;`);
...
bt.push(`export function getCurrentLocation(): Location { return currentLocation; }`);
bt.push(`export function setCurrentLocation(loc: Location): Location { return currentLocation = loc; }`);
bt.push(`export function setBabelTypes(_t: Object): Location { return t = _t; }`);
bt.push(``);
bt.push(`const BabelTypes = {`);
Object.keys(types.BUILDER_KEYS).sort().forEach(key => {
  bt.push(`  ${key[0].toLowerCase() + key.substr(1)}(${types.BUILDER_KEYS[key].map(field => {
    const t = getType(key, field);
  ...
  bt.push(`    const hasLoc = (loc && typeof loc === 'object' && typeof loc.start === 'object' && typeof loc.end === 'object');`);
  bt.push(`    if (hasLoc) {`);
  bt.push(`      args.pop();`);
  bt.push(`    }`);
  bt.push(`    return {...t.${key}.apply(t, args), loc: hasLoc ? (loc: any) : getCurrentLocation()};`);
  bt.push(`  },`);
});
Object.keys(types.BUILDER_KEYS).sort().forEach(key => {
...
Object.keys(types.BUILDER_KEYS).sort().forEach(key => {
  ...
  bt.push(`    return t.is${key}.apply(t, arguments) ? (value: any) : undefined;`);
  bt.push('  },');
});
bt.push(`}`);
bt.push(``);
